...
import { BlogMarkdownProps } from './type'
...
export const BlogMarkdown = ({ content }: BlogMarkdownProps) => {
  ...
  Boxコンポーネントを作成する場合は、~padding~や~margin~などのBoxでも使用する可能性が高いプロパティを持つコンポーネントを作成しておきます。そうすることで、次にボーダーを持つボックスが欲しくなったりした時に、作成したコンポーネントを[アンカーテキスト](https://zenn.dev)継承することでコード上に再定義する必要がなくなるので、コードの見通しや修正がしやすくなると感じていました。
